...
<!-- -------------- REPLACE PDF FILE -------------- -->
                <div class="form-row py-2" ID="upload_file">
                    ...
                    <div class="col-md">
                        ...
                        <span id="file_pdf_new" style="color: darkolivegreen; font-weight: bold; display:none">
                        </span>
                        ...
                        <ul>
                            @foreach ($files as $file)
                                <li class="px-2 py-1" id="options_{{$file->id}}">
                                    <label for="current_files" class="btn btn-light border">Anexar</label>
                                    <span class="px-3"></span>
                                    <span id="current" style="color: dimgrey">
                                        {{ $file->name }}
                                    </span>
                                    ...
                                    <input
                                        class="input"
                                        type="file" accept=".pdf, application/pdf"
                                        name="current_files" id="current_file"
                                        value="{{ $file->name }}"
                                        style="visibility: hidden">

                                    <span id="new_file" style="color: darkolivegreen; font-weight: bold; display:none"></span>
                                    <script>
                                        $("#current_file").css("display", "none");
                                        var input = document.getElementById('current_file');
                                        var infoArea;
                                        input.addEventListener( 'change', showFileName);
                                        function showFileName( event ) {
                                            var input = event.srcElement;
                                            var fileName = input.files[0].name;
                                            infoArea = document.getElementById('file_pdf_new');
                                            $("#current_file").css({"color": "darkred", "text-decoration": "line-through"});
                                            $("#new_file").css("display", "block");
                                            infoArea.textContent = (fileName);
                                        }
                                    </script>
                                    ...
                                    <a href="{{URL::to(route('files.destroy', $file->id))}}"
                                       data-toggle="tooltip" title="remover {{ $file->name }}"
                                           href="#" target="_blank"
                                           class="btn btn-light"
                                           name="files_to_remove[]"
                                           id="remove{{$file->id}}"
                                           onclick="if (confirm('Deseja deletar esse anexo?')){
...
<!-- -------------- UPLOAD FILES -------------- -->
                    @if ($count_anexos < 5) Anexar mais arquivos @endif
